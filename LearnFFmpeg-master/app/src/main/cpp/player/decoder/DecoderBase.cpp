...
int DecoderBase::InitFFDecoder() {
    int result = -1;
    LOGCATD("DecoderBase::InitFFDecoder m_Url : %s\n", m_Url);
    do {
        //1.创建封装格式上下文，解封装
        m_AVFormatContext = avformat_alloc_context();
        avformat_network_init();

        int avformat_open_input_result = 0;
        //2.打开文件
        if ((avformat_open_input_result = avformat_open_input(&m_AVFormatContext, m_Url, NULL,
                                                              NULL)) != 0) {
            LOGCATE("DecoderBase::InitFFDecoder avformat_open_input fail.");
            char szTemp[256];
            memset(szTemp, 0x00, sizeof(szTemp));
            av_strerror(avformat_open_input_result, szTemp, 255);
            LOGCATE("DecoderBase:: szTemp is : %s",szTemp);
            break;
        }
...
void DecoderBase::DoAVDecoding(DecoderBase *decoder) {
    ...
    do {
        if (decoder->InitFFDecoder() != 0) {
            LOGCATE("DecoderBase::Init ffmpeg Decoder in failure");
            break;
        }
